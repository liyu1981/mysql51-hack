...
THashNode* CHashMap::update_node(THashNode* node, void* new_data, int new_len, char* old_data, int* old_len)
{
	...
	if (old_chunk_num != new_chunk_num)
	{
		...
		BC_MEM_HANDLER_L head_hdr = allocator_.malloc(new_chunk_num);   //CHUNK数足够, 不会失败
		allocator_.split(head_hdr, new_data, new_len);
...
THashNode* CHashMap::update_node_metadata(THashNode* node, void* new_data, int new_len, long expiretime)
{
	int old_chunk_num = allocator_.get_chunk_num(node->chunk_len_);
	int new_chunk_num = allocator_.get_chunk_num(new_len);
...
void CHashMap::delete_from_node_list(THashNode* node)
{
	...
	if(next_node_hdr != INVALID_BC_MEM_HANDLER_32)
	{
...
void CHashMap::use_node(THashNode *node, TMBHashKey &key, int chunk_len, BC_MEM_HANDLER_L chunk_head)
{
	//set member
	node->key_ = key;
