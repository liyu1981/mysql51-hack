...
//
int CacheAccess::get(const char* key, char* buf, unsigned buf_size
			, unsigned& data_len, bool& dirty_flag, long& time_stamp, long& expiretime)
{
	//	find
	TMBHashKey hash_key;
	memcpy(hash_key._.md5_, key, TMBHashKey::C_DATA_LEN);
...
//	return <0 on error
//
int CacheAccess::get_key(const char* key, unsigned& data_len, bool& dirty_flag, long& time_stamp)
{
	//	find
	TMBHashKey hash_key;
	memcpy(hash_key._.md5_, key, TMBHashKey::C_DATA_LEN);
...
int CacheAccess::set(const char* key, const char* data, unsigned data_len, long expiretime, ENodeFlag flag/*=NODE_FLAG_DIRTY*/)
{
	//	find
	TMBHashKey hash_key;
	memcpy(hash_key._.md5_, key, TMBHashKey::C_DATA_LEN);
...
int CacheAccess::oldest_key(char* key, unsigned& data_len , bool& dirty_flag, long& time_stamp)
{
	THashNode* node = _cache.get_add_list_tail();
	if (node == NULL)
		return 1;
...
int CacheAccess::oldest_key(char* key, unsigned& data_len , bool& dirty_flag, long& time_stamp, long& expiretime)
{
	THashNode* node = _cache.get_add_list_tail();
	if (node == NULL)
		return 1;
...
int CacheAccess::open(char* mem, long mem_size, bool inited
					, int node_total, int bucket_size
					, int n_chunks, int chunk_size)
{
	if (mem_size < _cache.get_total_pool_size(node_total, bucket_size, n_chunks, chunk_size))
		return -1;
...
int CacheAccessUin::get_key(unsigned uin, unsigned& data_len, bool& dirty_flag, long& time_stamp)
{
	TMBHashKey hash_key;
	hash_key._.uin_ = uin;
...
int CacheAccessUin::core_dump_bucket_mem(char *buff, int maxsize, unsigned bucket_id, unsigned &core_size)
{
	return _da.core_dump_bucket_mem(buff, maxsize, bucket_id, core_size);
}
int CacheAccessUin::core_recover_bucket_mem(char *buff, int buffsize, unsigned &recover_size)
{
