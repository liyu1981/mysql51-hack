...
inline TChunkNode* CChunkAllocator::handler2ptr(BC_MEM_HANDLER_L handler)
{
    if (handler == INVALID_BC_MEM_HANDLER_L)
    {
        return NULL;
    }
...
inline BC_MEM_HANDLER_L CChunkAllocator::ptr2handler(TChunkNode* ptr)
{
    ...
    return (BC_MEM_HANDLER_L)(tmp_ptr - pool_);
}
